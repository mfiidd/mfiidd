...
The same number of 256-particle likelihood evaluations: nearly three hours.

. . .

That is the trade in one line. We spend simulation, and we buy back the
...
Locations agree.
Every NPE interval is [wider]{.alert} than the PMMH one, by one to fifty per
cent.

. . .

That is the direction the theory predicts, and the safer way to be wrong.

. . .

But agreement in location says nothing about whether the intervals sit in the
right place.
...
`D_lat` sits below the band: the estimator centres it too low.

. . .

A check MCMC never needed, because MCMC had convergence diagnostics instead.
A flow has none.
...
- find the parameter that calibration flags, and see why the width comparison missed it
