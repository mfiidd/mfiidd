...
## One factor per observation {.smaller}

After a resampling step every particle carries weight $1/J$ again. So at each
observation:

1. propagate each particle one day
2. weight it, $w_{t,j} = p(y_t \mid x_{t,j}, \theta)$
3. average, giving that day's factor
4. resample in proportion to $w_{t,j}$, which returns every weight to $1/J$

$$\hat{p}(y \mid \theta) = \prod_{t} \frac{1}{J}\sum_{j=1}^{J} w_{t,j}$$

. . .

Step 3 is an *unweighted* average, and can be, because step 4 left the particles
equally weighted. Earlier observations act through **which particles are still
there**, not through a weight carried forward.
...
::: {.cite}
Red diamonds are the data, yellow circles the particles, sized by weight. Struck
through when nobody resampled them.
:::
