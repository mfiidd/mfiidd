...
- **DIC**: the fit at the posterior mean, penalised by how much worse the fit is
  on average across the posterior

. . .

Both ask only for the likelihood of the whole series, which is what the filter
returns.
DIC is the criterion we can compute here, and a rough one: it approximates
predictive accuracy less well than the criteria usually recommended, which need
a likelihood term for each observation on its own.
...
$$\mathrm{DIC} = D(\bar{\theta}) + 2 p_D, \qquad p_D = \overline{D} - D(\bar{\theta})$$

- $D(\bar{\theta})$: the fit at the posterior mean $\bar{\theta}$
- $\overline{D}$: the fit averaged over the posterior
- $p_D$: how much worse the average is than the fit at the mean, the
  [effective number of parameters]{.alert}
