...
# When you cannot score the model

## Intractable does not mean hard

You can **run** the model. You cannot **score** it — evaluate
$p(y \mid \theta)$ for the data in front of you.

. . .

Not because the arithmetic is heavy. Because scoring it means summing over more
possibilities than there are.

## The particle filter was one way out {.smaller}

Yesterday: the state is hidden and random, so the likelihood is an integral, and
the filter estimated it.

. . .

That needed three things:

1. a state you can **simulate forward**
2. an observation density $p(y_t \mid x_t)$ you can **evaluate**
3. observations **through time**, to weight against

. . .

Take any one away and the filter has nothing to grip.

## Three models with no usable likelihood {.smaller}

- [**Final size only.**]{.alert} 273 of 284 islanders infected, 92 of them twice
  — and nothing about *when*. No time series, so nothing to filter along.

. . .

- [**A phylogeny.**]{.alert} You observe a pathogen tree built from sequences.
  Scoring it means summing over every transmission history consistent with that
  tree.

. . .

- [**An individual-based model.**]{.alert} Transmission over explicit households,
  schools, workplaces. A particle would have to be an entire population.

. . .

You can simulate all three. You cannot write down $p(y \mid \theta)$ for any of
them.
...
If you cannot write one down, choose the [distance directly]{.alert} instead.
