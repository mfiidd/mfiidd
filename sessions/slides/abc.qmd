...
## When you have to {.smaller}

You can always estimate $p(y \mid \theta)$: simulate, and average
$p(y \mid x_j, \theta)$ over the simulations. Two things go wrong.

. . .

[**The average degenerates.**]{.alert} $y$ is long, so nearly every simulation
misses it. That is the collapse you saw yesterday, and the particle filter fixed
it — by scoring one observation at a time.

. . .

[**There is nothing to average.**]{.alert} You have no
$p(y \mid x, \theta)$ to evaluate, only a sense of *close*: a tree shape, an
outbreak that "looked like this", a summary with no sampling distribution
attached.

. . .

ABC is for the second. Note what this is *not*: a stochastic model on its own is
no reason to reach for it — you spent two sessions estimating exactly that
likelihood.
