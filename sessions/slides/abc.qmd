...
For continuous data exact equality has probability zero. For our 59 counts it is
possible, but so unlikely that we would wait effectively forever. Two
...
- read an ABC rejection sampler and choose its threshold
- find a workable acceptance threshold, and see why that is the hard part
- trace ABC-SMC and compare it against rejection sampling
- see what a summary statistic and a distance each do to the posterior
- compare the ABC posterior against particle MCMC on the same three parameters, and on all six
