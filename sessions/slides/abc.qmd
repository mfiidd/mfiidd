...
You cannot evaluate $p(y \mid \theta)$. You can [simulate]{.alert} from it.

. . .

So: draw $\theta$ from the prior, simulate a dataset, and keep $\theta$ if the
simulation came out **equal to the data**.

. . .

That is not an approximation. The $\theta$ you keep are draws from the posterior.

## Two concessions {.smaller}
...
Both are forced on you. Neither is free.
...
sat from the prediction. Choosing a likelihood was choosing a distance; ABC only
