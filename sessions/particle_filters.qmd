...
2. **Transition dynamics** (`SEITLDynamics`) - how does the state evolve? (dispatches to the Gillespie stepper matching the number of compartments)
...
- `vcat(prior.init_state, 0.0)` appends `0.0` to the compartment vector to make it one element longer. We use that last slot to track daily incidence, because the observation process needs to see it. `collect(prev_state[1:(end - 1)])` drops it again and copies the compartments into a plain `Vector` — some inputs arrive as views or other array-like types, and the Gillespie steppers want a concrete vector.
:::
