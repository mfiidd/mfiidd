...
We use the Gillespie algorithm introduced in the [SEITL session](seitl.qmd), adapted here for the SEIT4L model with its four T compartments.
The implementation below is shown from the package source rather than repeated by
hand, so that the page and the code cannot drift apart. This is the function
`SEIT4LDynamics` calls once per particle per day:

::: {.callout-note collapse="true"}
## Gillespie step implementation

```{julia}
#| echo: false
#| output: asis
println("```julia\n", source_for(gillespie_step), "\n```")
```
:::
...
Here is the full interface implementation (defined in [`src/seit4l_ssm_interface.jl`](https://github.com/mfiidd/mfiidd/blob/main/src/seit4l_ssm_interface.jl)). Each type carries a docstring spelling out its state vector and the method it implements — useful for reference once you've read the prose above.
...
Gillespie's algorithm steps one event at a time, so a day holding two hundred events costs two hundred draws.
...
[`JumpProcesses.jl`](https://docs.sciml.ai/JumpProcesses/stable/) offers both kinds.
...
Simulating 59 days of SEIT4L against the Gillespie stepper this course uses gives:

| Population | Gillespie | Adaptive tau-leaping | Speed-up |
|-----------:|----------:|---------------------:|---------:|
| 284 | 75 µs | 90 µs | 0.8× |
| 2 840 | 868 µs | 162 µs | 5× |
| 28 400 | 7.4 ms | 79 µs | 94× |
| 284 000 | 70 ms | 92 µs | 755× |
...
Over 2000 runs the mean final size came out at 210 under adaptive tau-leaping against 197 under exact simulation.
