...
`GeneralisedFilters.jl` provides a family of algorithms behind the interface used above, and only part of that family applies to a model like this one.
...
That one fact decides which filters are open to you.

**Why the bootstrap filter is the one that works.**
...
Substituting `ParticleFilter(n, LatentProposal())` for `BF(n)` gives the same log-likelihood, because it is the same algorithm.
That cancellation is the point: the bootstrap filter is the member of the family that never has to evaluate the transition density.

**What a simulate-only model gives up.**
The usual way to make a particle filter cheaper is a proposal that looks at the observation before propagating, which spreads the weights less and so needs fewer particles.
Forming that weight means evaluating the transition density, and `AuxiliaryParticleFilter` needs the same density for its look-ahead score.
Both fail here with a `MethodError` on `SSMProblems.distribution` for the dynamics.
So a better proposal is not available to buy a lower particle count with, which is why the calibration section buys it with particles.
...
The resampling scheme is swappable through the `resampler` keyword.
