...
`SEIT4LDynamics` calls once per particle per day, and the stoichiometry table and
the rate function it uses sit beside it:
...
- **Stoichiometry table**: Defines how each transition changes the state vector. It is a
  constant rather than something the stepper builds, because the stepper runs once per
  particle per day and an allocation there is millions of allocations in a filter run
