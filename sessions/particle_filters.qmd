...
2. **Transition dynamics** (`SEITLDynamics`) - how does the state evolve? (uses the Gillespie stepper for however many T sub-stages the state carries)
...
The `run_particle_filter` function assembles the SSM components and runs the bootstrap filter. The initial state sets how many T sub-stages there are, so the same function serves any of them:
