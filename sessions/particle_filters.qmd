...
#| echo: false
#| output: asis
println("```julia\n", source_for(gillespie_step_seit4l), "\n```")
```
:::
...
for day in 1:5
    for i in 1:n_particles
        particles[i], inc = gillespie_step_seit4l(Random.default_rng(), particles[i], θ_example)
...
2. **Transition dynamics** (`SEITLDynamics`) - how does the state evolve? (the stage count is declared when it is built, and dispatch picks the Gillespie stepper from it)
