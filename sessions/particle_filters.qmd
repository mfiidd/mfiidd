...
That cancellation is also what makes it the filter available here, because it is the one member of the family that never has to evaluate the transition density.
A proposal that looks at the observation before propagating would spread the weights less, and `AuxiliaryParticleFilter` would reweight by a look-ahead score, but both need that density and both stop at a `MethodError` on `SSMProblems.distribution`.
...
Substituting `BF(n_particles; threshold = 0.5, resampler = Multinomial())` for `BF(n_particles)` in the runner turns that argument into something you can measure with the calibration cell, and `threshold = 0.0`, which never resamples at all, puts the degeneracy back so you can see what it costs.
