...
hand, so that the page and the code cannot drift apart. `gillespie_step!` is what
`SEIT4LDynamics` calls once per particle per day, advancing a state vector the caller
owns. `gillespie_step` is the same simulator with the copy taken for you, and is the
one used in the propagation example below:
...
- **Stoichiometry table**: How each transition changes the state vector, held as a
  constant so the stepper does not rebuild it on every call
