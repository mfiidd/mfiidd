...
Rather than an algorithm written out by hand, the model is given as a list of transitions and their rates, and [JumpProcesses.jl](https://docs.sciml.ai/JumpProcesses/stable/) samples it exactly.
The transitions are shown from the package source rather than repeated by hand,
so that the page and the code cannot drift apart. The number of temporary
...
- `vcat(prior.init_state, 0.0)` appends `0.0` to the compartments to make the state one element longer. That last slot tracks daily incidence, because the observation process needs to see it. `collect(prev_state[1:(end - 1)])` takes everything except that slot and copies it into a plain `Vector` — some inputs arrive as views or other array-like types, and the simulator wants a concrete vector. Writing it as `end - 1` rather than a fixed 8 is what lets the same code run SEITL and SEIT4L.
:::
