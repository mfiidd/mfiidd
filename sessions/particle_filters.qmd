...
`SEITLDynamics` calls once per particle per day:
...
2. **Transition dynamics** (`SEITLDynamics`) - how does the state evolve? (uses `gillespie_step` for SEIT4L, and the five compartment stepper for SEITL)
...
- `vcat(prior.init_state, 0.0)` appends `0.0` to the compartment vector to make it one element longer. We use that last slot to track daily incidence, because the observation process needs to see it. `collect(prev_state[1:(end - 1)])` drops it again and copies the compartments into a plain `Vector` — some inputs arrive as views or other array-like types, and the Gillespie steppers want a concrete vector. Writing the slice as `end - 1` rather than a fixed length is what lets one set of types serve both models: SEIT4L starts from eight compartments and SEITL from five, and the interface never needs to know which it has.
:::

How the pieces fit together:

- The state vector is [S, E, I, T1, T2, T3, T4, L, daily_inc] for SEIT4L, and [S, E, I, T, L, daily_inc] for SEITL - the last element tracks daily incidence for the observation process either way
- `SEITLDynamics` wraps the Gillespie simulator, calling one step of it each day
- `PoissonObservation` assumes observed cases follow Poisson(ρ × true_incidence)

## Running the bootstrap particle filter

The `run_particle_filter` function assembles the SSM components and runs the bootstrap filter. The initial state decides which model is filtered, so the same function serves SEITL and SEIT4L:
