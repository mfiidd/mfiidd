...
We use the Gillespie algorithm introduced in the [SEITL session](seitl.qmd).
The stepper takes the number of temporary immunity stages `k`, so `k = 1` is SEITL and `k = 4` the SEIT4L model.
...
- **Transitions**: Each moves one individual between two compartments, so a pair of indices gives its effect on the state vector
...
for day in 1:5
    for i in 1:n_particles
        particles[i], inc = gillespie_step(Random.default_rng(), particles[i],
                                           θ_example; k = 4)
...
2. **Transition dynamics** (`SEITLDynamics`) - how does the state evolve? (calls `gillespie_step` with the number of immunity stages it carries)
...
- `vcat(prior.init_state, 0.0)` appends `0.0` to the compartment vector to make it one element longer. We use that last slot to track daily incidence, because the observation process needs to see it. `collect(prev_state[1:(end - 1)])` drops it again and copies the compartments into a plain `Vector` — some inputs arrive as views or other array-like types, and `gillespie_step` wants a concrete vector.
:::
