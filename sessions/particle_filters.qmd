...
`GeneralisedFilters.jl` provides a family of algorithms behind the interface used above, and what a model can do decides which of them are open to it.
A Gillespie simulator can be sampled from, but its transition density cannot be evaluated, and the dynamics defined above implement `simulate` and nothing else.
...
`AuxiliaryParticleFilter` reweights by a look-ahead score instead, and whether it runs depends on how that score is formed: `MeanPredictive` and `ModePredictive` evaluate the density and fail the same way, while `DrawPredictive` draws a step forward by simulation and runs here.
...
Neither fits a Gillespie compartmental model, but both are worth knowing before reaching for particles.
